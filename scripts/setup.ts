...
export async function setupEnvironment(opts: SetupOptions): Promise<TestEnvironment> {
  ...
  const pk_eth_deployer: string =
    opts.pk_eth_deployer || '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80';
  const pk_eth_signer1: string =
    opts.pk_eth_signer1 || '0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d';
  const pk_eth_signer2: string =
    opts.pk_eth_signer2 || '0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a';
  const pk_fuel_deployer: string =
    opts.pk_fuel_deployer || '0xa449b1ffee0e2205fa924c6740cc48b3b473aa28587df6dab12abc245d1f5298';
  const pk_fuel_signer1: string =
    opts.pk_fuel_signer1 || '0x6fbaf19e3e42af0becc628e954557a8616d702b0c7ce9f2dd2580a16bebe357a';
  const pk_fuel_signer2: string =
    opts.pk_fuel_signer2 || '0xd4de6ad33dae7d7987711360b8ff9298f1838c9542c6efafa0e8c74b11e5623d';
  ...
  if (fuel_deployerBalance.lt(fuels_parseEther('5'))) {
    throw new Error(
      'Fuel deployer balance is very low (' + fuels_formatEther(fuel_deployerBalance) + 'ETH)'
    );
  }
  ...
  let eth_l1ERC20Gateway: L1ERC20Gateway = L1ERC20Gateway__factory.connect(
    eth_l1ERC20GatewayAddress,
    eth_deployer
  );
